...
import (
	...
	"github.com/crossplane-contrib/provider-argocd/pkg/version"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Argo CD support for Crossplane.").DefaultEnvars()
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-argocd:%s", version.Version))),
		}
	...
	kingpin.FatalIfError(customresourcesgate.Setup(mgr, o), "Cannot setup custom resources gate controller")
